...
fun DokterScreen(viewModel: DokterViewModel) {
    ...
    Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
        ...
        BasicTextField(value = nama, onValueChange = { nama = it }, modifier = Modifier.fillMaxWidth().padding(8.dp),
        ...
        BasicTextField(value = spesialis, onValueChange = { spesialis = it }, modifier = Modifier.fillMaxWidth().padding(8.dp),
            decorationBox = { innerTextField ->
                Box(modifier = Modifier.padding(8.dp)) {
        ...
        Button(onClick = {
            viewModel.insertDokter(Dokter(0, nama.text, spesialis.text, klinik.text, noHp.text, jamKerja.text))
        }, modifier = Modifier.padding(8.dp)) {
            Text("Simpan")
        }

        LazyColumn(modifier = Modifier.fillMaxWidth()) {
            items(viewModel.getAllDokter().value ?: emptyList()) { dokter ->
